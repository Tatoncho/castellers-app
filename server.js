...
app.get('/api/reset-total', async (req, res) => {
  if (req.query.confirm !== 'si') {
    return res.json({
      success: false,
      aviso: 'Esto BORRA la tabla castellers entera y la crea de nuevo. Añade ?confirm=si a la URL para confirmar, ej: /api/reset-total?confirm=si'
    });
  }
  ...
  res.json({
    ...
    detalle: resultados,
    aviso: 'Tabla recreada. Borra /api/reset-total del server.js ahora que ya la has usado. Después de importar el CSV, visita una vez /api/fix-id-sequence.'
  });
});
